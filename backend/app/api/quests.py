...
class CompletionCheckPublic(BaseModel):
    type: str
    min_count: int


class CompletionRequirements(BaseModel):
    """completion_condition.checks를 scope 기준으로 나눠 공개한다.

    프론트가 my_response_status/unmet_check_types만 보고 "몇 번 해야 끝나는지"를
    추론하지 않도록, 필요한 체크 목록(min_count 포함) 자체를 그대로 알려준다.
    """

    member_checks: list[CompletionCheckPublic]
    team_checks: list[CompletionCheckPublic]
...
def _build_response(assignment: QuestAssignment, actor: RoomActor, db: DBSession) -> QuestCurrentResponse:
    ...
    return QuestCurrentResponse(
        ...
        completion_requirements=CompletionRequirements(
            member_checks=[
                CompletionCheckPublic(type=c["type"], min_count=c.get("min_count", 1))
                for c in checks
                if c.get("scope") == "PER_MEMBER"
            ],
            team_checks=[
                CompletionCheckPublic(type=c["type"], min_count=c.get("min_count", 1))
                for c in checks
