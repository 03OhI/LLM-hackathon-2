...
5점 리커트 채점 — SURVEY24.md v2 정본.
  ...
  4) 이진 변환      r > 0.55 → 상위 극 / r < 0.45 → 하위 극 / 사이 → 중립(None)
...
# 중립대의 양 끝 — 이 파일이 단일 정의처다. `chemistry_v2.score_drive()` 의 cap/sup
# 판정도 여기서 import 해서 쓴다. 예전에는 양쪽에 0.60/0.40 이 따로 하드코딩돼 있어서,
# 한쪽만 좁혔더니 주도성 판정 불가율이 58.8% 그대로였다(라벨은 붙는데 계산에서 빠짐).
#
# 2026-08-28 0.60/0.40 → 0.55/0.45 로 축소 [팀 판단]
#   실제 응답 22명 기준 팀 진단 불가 67.0% → 23.8%.
#   대가: 5점 척도 평균 3.2 인 사람이 '캡틴'으로 분류된다. 약한 신호를 강한 신호로
#   재분류하는 것이고 판정 신뢰도는 내려간다. 근본 원인은 문항의 축 분리 실패(α<.60)이며
#   이 값은 그 증상 완화다. 대안이던 게이트 완화(MIN_JUDGED 3→2)는 판정 인원 2명 이하로
#   주도성을 판정하는 팀을 36.8% 만들어 폐기했다.
HI, LO = 0.55, 0.45          # 이진 변환 경계 (평균 3.2 / 2.8)
