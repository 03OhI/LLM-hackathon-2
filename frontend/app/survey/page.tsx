...
type SavedTeamSession = { team: TeamSession; phase: "lobby" | "survey" | "waiting" };
type DemoResultSession = { displayName: string; teamName: string; responses: Record<string, number> };
type TeamStatus = { completedMembers: number; expectedMembers: number; joinedMembers: number; members: { displayName: string; completed: boolean }[]; ready: boolean };
...
function saveTeamSession(team: TeamSession, phase: SavedTeamSession["phase"]) {
  if (typeof window !== "undefined") window.sessionStorage.setItem(teamSessionStorageKey, JSON.stringify({ team, phase }));
}

function getSavedTeamSession(): SavedTeamSession | null {
  if (typeof window === "undefined") return null;
  try { const saved = JSON.parse(window.sessionStorage.getItem(teamSessionStorageKey) ?? "null") as SavedTeamSession | null; return saved?.team?.code && saved.team.memberId ? saved : null; } catch { return null; }
}

function saveDemoResult(result: DemoResultSession) {
  if (typeof window !== "undefined") window.sessionStorage.setItem(demoResultStorageKey, JSON.stringify(result));
}

function WaitingScreen({ team }: { team: TeamSession }) {
  const [status, setStatus] = useState<TeamStatus>({ completedMembers: 1, expectedMembers: team.expectedMembers, joinedMembers: 1, members: [], ready: false });
  const [copied, setCopied] = useState(false);
  useEffect(() => { const refresh = async () => { try { const response = await fetch(`/api/teams/${team.code}`, { cache: "no-store" }); if (response.ok) setStatus(await response.json()); } catch { /* 프론트 시연에서는 대기 UI를 계속 보여 줍니다. */ } }; refresh(); const timer = window.setInterval(refresh, 3000); return () => window.clearInterval(timer); }, [team.code]);
  useEffect(() => { if (status.ready) window.location.replace(`/results?team=${team.code}&member=${team.memberId}`); }, [status.ready, team.code, team.memberId]);
const invite = typeof window === "undefined" ? "" : `${window.location.origin}/?team=${team.code}`;
  return <main className="survey-shell"><SurveyNav /><section className="survey-card waiting-card survey-card-enter"><div className="waiting-duck" aria-hidden="true">{[1, 2, 3, 4].map((frame) => <Image key={frame} className={`waiting-frame waiting-frame-${frame}`} src={`/wait-duck-${frame}.png`} alt="" width={250} height={250} priority />)}</div><p className="eyebrow">TEAM ANALYSIS</p><h1>{status.ready ? "결과를 준비하고 있어요." : "팀원들의 응답을 모으고 있어요."}</h1><p className="waiting-count">로딩 중 <b>({status.completedMembers} / {status.expectedMembers})</b></p><div className="waiting-progress"><i style={{ width: `${(status.completedMembers / status.expectedMembers) * 100}%` }} /></div><p className="waiting-note">모든 팀원이 설문을 마치면 팀 분석 결과가 준비됩니다.</p><div className="waiting-actions"><button className="invite-button" onClick={async () => { await navigator.clipboard?.writeText(invite); setCopied(true); }}>{copied ? "참여 링크가 복사되었어요" : "팀 참여 링크 복사"}</button><button className="admin-preview-button" onClick={() => window.location.assign("/results?demo=1")}>관리자 · 결과 미리보기 <span aria-hidden="true">→</span></button></div><p className="waiting-demo-note">시연용 버튼이며, 현재 기기에서 작성한 응답으로 결과를 보여 줍니다.</p></section></main>;
}

function TeamLobby({ team, onStart }: { team: TeamSession; onStart: () => void }) {
  const [status, setStatus] = useState<TeamStatus>({ completedMembers: 0, expectedMembers: team.expectedMembers, joinedMembers: 1, members: [], ready: false });
  const [copied, setCopied] = useState(false);
...
const invite = typeof window === "undefined" ? "" : `${window.location.origin}/?team=${team.code}`;
  ...
  useEffect(() => { if (invite) QRCode.toDataURL(invite, { width: 340, margin: 1, color: { dark: "#334166", light: "#FFFFFF" } }).then(setQrCode); }, [invite]);
  return <main className="survey-shell"><SurveyNav /><section className="survey-card team-lobby survey-card-enter"><div className="lobby-heading"><div><div className="intro-kicker"><Image src="/duck-face-open.png" alt="" width={48} height={48} /><span>{team.teamName}</span></div><h1>팀원을 초대해 주세요</h1><p>링크 또는 QR을 공유하면 팀원들이 바로 설문에 참여할 수 있어요.</p></div><Image className="lobby-face" src="/duck-face-sparkle.png" alt="" width={130} height={130} priority /></div><div className="lobby-grid"><section className="lobby-invite"><p className="lobby-label">TEAM INVITE</p>{qrCode ? <Image className="invite-qr" src={qrCode} alt={`${team.teamName} 설문 참여 QR 코드`} width={170} height={170} unoptimized /> : <div className="invite-qr loading" />}<p className="invite-code">{team.code}</p><button className="invite-button lobby-copy" onClick={async () => { await navigator.clipboard?.writeText(invite); setCopied(true); }}>{copied ? "초대 링크가 복사되었어요" : "초대 링크 복사"}</button></section><section className="lobby-members"><div className="lobby-member-heading"><div><p className="lobby-label">TEAM STATUS</p><h2>참여 현황 <b>{status.joinedMembers} / {status.expectedMembers}</b></h2></div><button className="lobby-refresh" onClick={async () => { const response = await fetch(`/api/teams/${team.code}`, { cache: "no-store" }); if (response.ok) setStatus(await response.json()); }} aria-label="참여 현황 새로고침">↻</button></div><ul>{Array.from({ length: status.expectedMembers }, (_, index) => { const member = status.members[index]; return <li className={member ? "joined" : "waiting"} key={index}><i>{member ? (member.completed ? "✓" : "●") : ""}</i><span>{member?.displayName ?? "팀원 참여 대기"}</span><small>{member ? (member.completed ? "설문 완료" : "설문 전") : "대기"}</small></li>; })}</ul></section></div><div className="lobby-note"><Image src="/duck-face-smile.png" alt="" width={56} height={56} /><p>팀원들이 링크로 참여하면 이 화면에서 참여 현황이 자동으로 업데이트돼요.</p></div><div className="survey-actions"><Link className="button-muted" href="/">나중에 하기</Link><button className="button-next intro-start-button" onClick={onStart}><span>내 설문 시작하기</span><i aria-hidden="true">→</i></button></div></section></main>;
}
