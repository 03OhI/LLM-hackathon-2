...
const axisMeta: Record<AxisKey, { high: string; low: string; color: string; light: string }> = {
  plan: { high: "계획을 먼저 세움", low: "일단 시작해 봄", color: "#e4a642", light: "#fff7e8" },
  drive: { high: "앞에서 이끔", low: "뒤에서 받쳐 줌", color: "#ed846d", light: "#fff2ee" },
  conflict: { high: "문제를 바로 짚음", low: "관계를 먼저 조율함", color: "#8ca78b", light: "#f0f6ee" },
  communication: { high: "결론부터 말함", low: "배경부터 설명함", color: "#84a7c4", light: "#f0f6fb" },
};
...
function PersonalResult({ personal, onTeam }: { personal: Personal; onTeam: () => void }) {
  const type = personal.result.type;
  const axes = personal.result.axes;
  return <section className="result-report personal-report survey-card-enter">
    <header className="personal-hero"><div><p className="result-kicker">{personal.displayName}님의 협업 경향</p><h1>{type ? type.alias : "나만의 협업 균형"}</h1><span>{type && personal.result.code ? `${personal.result.code} · 네 가지 축이 보여 주는 현재의 일하는 방식이에요.` : "네 가지 축 중 일부가 한쪽으로 뚜렷하지 않아요. 상황에 따라 유연하게 움직이는 편이에요."}</span></div><Image src="/duck-face-sparkle.png" alt="기쁜 표정의 TMTI 오리" width={132} height={132} priority /></header>
    <section className="personal-overview" aria-labelledby="radar-title"><div className="radar-copy"><p>네 가지 협업 축</p><h2 id="radar-title">한쪽이 좋다는 뜻은 아니에요.</h2><span>팀에서 어떤 방식이 편한지, 서로 어떻게 맞춰 갈지 확인하는 지도입니다.</span><div className="radar-legend">{axisOrder.map((axis) => <span key={axis}><i style={{ background: axisMeta[axis].color }} />{axes[axis].label}</span>)}</div></div><RadarChart axes={axes} /></section>
    <section className="axis-map" aria-label="협업 축별 위치"><header><div><p>나의 협업 지도</p><h2>각 축에서 편한 방향</h2></div><span>상황에 따라 달라질 수 있어요</span></header>{axisOrder.map((axis) => <AxisPosition key={axis} axis={axis} score={axes[axis]} />)}</section>
    <section className="personal-actions" aria-label="협업 제안"><article className="insight-strength"><span>잘 발휘되는 점</span><p>{type?.strength ?? "한쪽에 고정되지 않아, 팀 상황에 맞춰 역할을 조정할 수 있어요."}</p></article><article className="insight-caution"><span>함께 맞춰 볼 점</span><p>{type?.caution ?? "내가 편한 방식과 다른 팀원의 방식이 있을 수 있음을 먼저 확인해 보세요."}</p></article><article className="insight-role"><span>잘 맞는 자리</span><p>{type?.role ?? "회의에서 서로의 방식과 필요한 역할을 함께 정해 보세요."}</p></article></section>
    <button className="result-team-link" onClick={onTeam}>우리 팀 분석 보기 <b>→</b></button>
  </section>;
}
...
function RadarChart({ axes }: { axes: Personal["result"]["axes"] }) {
  const center = 140;
  const radius = 104;
  const points = axisOrder.map((axis, index) => { const angle = -Math.PI / 2 + index * Math.PI / 2; const length = Math.max(18, axes[axis].ratio * radius); return [round(center + Math.cos(angle) * length), round(center + Math.sin(angle) * length)] as const; });
  const polygon = points.map(([x, y]) => `${x},${y}`).join(" ");
  const grid = (scale: number) => axisOrder.map((_, index) => { const angle = -Math.PI / 2 + index * Math.PI / 2; return `${round(center + Math.cos(angle) * radius * scale)},${round(center + Math.sin(angle) * radius * scale)}`; }).join(" ");
  return <div className="radar-chart" role="img" aria-label="계획성, 주도성, 갈등 대응, 소통 직접성의 개인 협업 경향 그래프"><svg viewBox="0 0 280 280" aria-hidden="true"><polygon className="radar-grid" points={grid(1)} /><polygon className="radar-grid radar-grid-inner" points={grid(.5)} />{axisOrder.map((_, index) => { const angle = -Math.PI / 2 + index * Math.PI / 2; return <line key={index} x1={center} y1={center} x2={center + Math.cos(angle) * radius} y2={center + Math.sin(angle) * radius} />; })}<polygon className="radar-shape" points={polygon} />{points.map(([x, y], index) => <circle key={axisOrder[index]} cx={x} cy={y} r="5" style={{ fill: axisMeta[axisOrder[index]].color }} />)}</svg>{axisOrder.map((axis, index) => <span key={axis} className={`radar-label label-${index}`} style={{ color: axisMeta[axis].color }}>{axes[axis].label}</span>)}</div>;
}

function AxisPosition({ axis, score }: { axis: AxisKey; score: Personal["result"]["axes"][AxisKey] }) {
